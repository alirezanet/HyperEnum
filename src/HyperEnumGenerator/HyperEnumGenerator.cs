...
namespace HyperEnumGenerator
{
    ...
    {
        private record EnumInfo(string Name, string FullName, IEnumerable<string> Members);
        ...
        {
            var enumInfos = GetEnums(context).ToList();
            if (enumInfos.Count <= 0) return;

            GenerateHyperEnumExtensions(context, enumInfos);
            GenerateHyperEnumClass(context, enumInfos);
        }

        private static void GenerateHyperEnumClass(GeneratorExecutionContext context, List<EnumInfo> enumInfos)
        {
            ...
            writer.WriteLine("{");
            writer.Indent++;
            writer.WriteLine("public static class HyperEnumHelper");
            writer.WriteLine("{");
            writer.Indent++;

            foreach (var (name, _, members) in enumInfos)
                WriteGetNamesMethods(name, members, writer);

            writer.Indent--;
            writer.WriteLine("}");
            writer.Indent--;
            writer.WriteLine("}");
            context.AddSource("HyperEnumHelper.Generated", output.ToString());
        }

        private static void WriteGetNamesMethods(string name, IEnumerable<string> members, IndentedTextWriter writer)
        {
            writer.WriteLine($"public static string[] Get{name}Names()");
            writer.WriteLine("{");
            writer.Indent++;
            var commaSeperatedStrings = string.Join(",", members.Select(q => $"\"{q}\""));
            writer.WriteLine($"return new[] {{ {commaSeperatedStrings } }};");

            writer.Indent--;
            writer.WriteLine("}");
        }

        private static void GenerateHyperEnumExtensions(GeneratorExecutionContext context,
            List<EnumInfo> enumItems)
        {
            using var output = new StringWriter();
            using var writer = new IndentedTextWriter(output);
            writer.WriteLine("//<auto-generated/>");
            ...
            foreach (var (_, fullName, members) in enumItems)
                WriteExtensionMethod(fullName, members, writer);

            writer.Indent--;
            writer.WriteLine("}");
            writer.Indent--;
            writer.WriteLine("}");
            context.AddSource("HyperEnumExtensions.Generated", output.ToString());
        }

        private static void WriteExtensionMethod(string fullName, IEnumerable<string> members,
            IndentedTextWriter writer)
        {
            writer.WriteLine($"public static string GetName(this {fullName} member)");
            writer.WriteLine("{");
            writer.Indent++;
            ...
            foreach (var member in members)
                writer.WriteLine($"{fullName}.{member} => \"{member}\",");
        ...
        private static IEnumerable<EnumInfo> GetEnums(
        ...
        private static IEnumerable<EnumDeclarationSyntax> GetEnumItems(SyntaxTree syntaxTree)
        {
            return syntaxTree.GetRoot().DescendantNodes().OfType<EnumDeclarationSyntax>()
                .Where(x => x.AttributeLists.Any(q =>
                    Regex.IsMatch(q.ToString(), @"\[\s*?(?:HyperEnum|HyperEnumAttribute)\s*?\]")));
        }

        private static EnumInfo ParseEnum(EnumDeclarationSyntax enumDeclarationSyntax)
        {
            return new EnumInfo(enumDeclarationSyntax.Identifier.Text, GetEnumFullName(enumDeclarationSyntax),
